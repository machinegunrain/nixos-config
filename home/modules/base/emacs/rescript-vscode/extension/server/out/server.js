...
let findBinaryPathFromProjectRoot = (directory // This must be a directory and not a file!
) => {
...
let findBscBinary = (projectRootPath) => {
    let rescriptBinaryPath = findRescriptBinary(projectRootPath);
    if (rescriptBinaryPath !== null) {
        let rescriptDirPath = path.join(path.dirname(rescriptBinaryPath), "..", "rescript");
        ...
        if (process_1.default.platform == "darwin" &&
            ...
            !fs_1.default.existsSync(bscBinaryPath)) {
            bscBinaryPath = path.join(rescriptDirPath, process_1.default.platform, c.bscExeName);
        }
        ...
        if (!initialized && msg.method !== "initialize") {
            ...
            let result = {
                ...
                capabilities: {
                    ...
                    inlayHintProvider: (_b = extensionConfiguration.inlayHints) === null || _b === void 0 ? void 0 : _b.enable,
